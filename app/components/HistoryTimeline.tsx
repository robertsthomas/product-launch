...
export function HistoryTimeline({ entries, maxEntries = 10, showEmpty = true }: HistoryTimelineProps) {
  ...
  if (entries.length === 0 && showEmpty) {
    return (
      ...
      >
        ...
        <div style={{ fontSize: "var(--text-sm)" }}>Changes will appear here after audits and fixes</div>
      </div>
    )
  }
...
function getDefaultDescription(entry: HistoryEntry): string {
  switch (entry.changeType) {
    ...
    case "autofix":
      return entry.changedField ? `Auto-fixed ${formatFieldName(entry.changedField)}` : "Applied auto-fix"
    case "ai_fix":
      return entry.changedField ? `AI generated ${formatFieldName(entry.changedField)}` : "AI content generated"
    case "manual_edit":
      return entry.changedField ? `Manually updated ${formatFieldName(entry.changedField)}` : "Manual edit"
    case "bulk_fix":
      return entry.changedField ? `Bulk updated ${formatFieldName(entry.changedField)}` : "Bulk fix applied"
    default:
