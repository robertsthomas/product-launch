...
export function analyzeImage(image: {
  ...
  if (image.width && image.height) {
    ...
    const matchedRatio = RECOMMENDED_RATIOS.find(
      r => Math.abs(ratio - r.ratio) <= r.tolerance
    );
 ...
 */
export function analyzeProductImages(images: Array<{
  id: string;
  url: string;
  altText: string | null;
  width?: number;
  ...
  const overallScore = Math.round(
    analyzedImages.reduce((sum, img) => sum + img.score, 0) / analyzedImages.length
  );

  // Generate summary
  let summary: string;
  if (criticalIssues > 0) {
    const missingAlt = analyzedImages.filter(
 ...
 */
export function getImagesNeedingAltText(images: Array<{
  id: string;
  url: string;
  altText: string | null;
